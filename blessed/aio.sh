...
case "$1" in
  ...
  ;;
	...
	;;
	 ...
	 mklns)
	   ...
	   # sh $0 bsp/ bsp2/ OR
	   #/cygdrive/c/cygwin/home/xxxx/data/bsp1/ [path]/data/bsp2/
   ...
   trmxssh)
	   ...
	   cd ~/.ssh
	   sshpass -p 'xxxx2' ssh xxxx@192.168.1.2 -p 2222
   ;;
   # ###### rsync
   pc2trmx)
	   ...
	   rsync -av -e 'ssh -p 2222' xxxx@192.168.1.2:~/synk/admn-pc.txt  rsynced/
	   #passphrase C.....2
	;;
   trmx2pc)
	   ...
	   # MobaXterm starten port 2222
	   # ~/ ist my HOME unter MobaXterm
	   rsync -av -e 'ssh -p 2222' rsynced/ xxxx@192.168.1.2:~/synk/
	   # passphrase C.....2
	;;
	rsyncssh)
  ...
  ;;
    ...
    rsync -aubPvlzrn --exclude=excl*  --backup-dir=_bkp_ --log-file="/drives/c/Users/xxxx/Desktop/rsync/log/rsync.$(date +%d.%m).log" "/drives/c/Users/xxxx/Desktop/rsync/rsyncsrc/" "/drives/c/Users/xxxx/Desktop/rsync/rsyncdest/" --delete
